...
export class SetScheduleComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    for (let i = 1; i <= 8; ++i) {
      ...
      if (startTime < 10) {
        ...
        if (++startTime < 10) end_time = '0' + startTime + ':00:00';
        else end_time = startTime + ':00:00';
      } else {
  ...
  toggleSelected(event: Event) {
    if (this.timeFrames.isSelected == false) this.timeFrames.isSelected = true;
    else this.timeFrames.isSelected = false;
  }
  OnSetSchedule() {
    this.selectedTime = [];
